...
describe('PlayerController', () => {
    ...
    const builder = Test.createTestingModule({
      ...
      providers: [
        { provide: PlayerService, useValue: {
          addPlayer: jest.fn(), updatePlayerStatus: jest.fn(),
        } },
  ...
  describe('addPlayer', () => {
    it('should throw when file missing', async () => {
      await expect(controller.addPlayer(undefined as any, { name: 'P', dob: '2010-01-01', jerseyNumber: 7 } as any, { user: { coachId: 'c1' } } as any)).rejects.toThrow(BadRequestException);
    });

    it('should call service and return response', async () => {
      service.addPlayer.mockResolvedValue({ message: PLAYER_MESSAGES.CREATED, player: { id: 'p1' } } as any);
      const res = await controller.addPlayer(mockFile, { name: 'P', dob: '2010-01-01', jerseyNumber: 7 } as any, { user: { coachId: 'c1' } } as any);
      expect(service.addPlayer).toHaveBeenCalledWith('c1', { name: 'P', dob: '2010-01-01', jerseyNumber: 7 }, mockFile);
