...
export class PlayerService {
  ...
  private validatePhoto(file: Express.Multer.File) {
    ...
    if (!allowed.includes(file.mimetype)) throw new BadRequestException(PLAYER_MESSAGES.INVALID_FILE_TYPE);
    if (file.size > 2 * 1024 * 1024) throw new BadRequestException(PLAYER_MESSAGES.FILE_TOO_LARGE);
  }
  ...
  if (count >= 15) throw new BadRequestException(PLAYER_MESSAGES.TEAM_LIMIT_EXCEEDED);

  const jerseyExists = await this.playerRepo.findByJersey(team.id, dto.jerseyNumber);
  if (jerseyExists) throw new BadRequestException(PLAYER_MESSAGES.DUPLICATE_JERSEY);

  const age = this.calculateAgeFromDob(dto.dob);
  if (age < 10 || age > 18) throw new BadRequestException(PLAYER_MESSAGES.AGE_LIMIT);
  ...
  async updatePlayerStatus(dto: UpdatePlayerStatusDto) {
    ...
    if (coachData?.coach.user.email) {
      ...
      await this.mailerService.sendMail(
        ...
        `
        <p>Dear ${coachData?.coach.user.name},</p>
        <p>The player "<strong>${playerData.name}</strong>" has been <strong>${statusText}</strong> by the admin.</p>
